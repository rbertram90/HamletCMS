{viewCrumbtrail(array("/overview/{$blog['id']}", $blog['name'], "/config/{$blog['id']}", 'Settings'), 'Customise Header')}
...
<form action="/config/{$blog.id}/header/submit" method="post" id="frm_updateheader">
    ...
    <div style="margin-bottom:20px;">
        ...
        <button type="button" title="Insert Image" onclick="rbrtf_showWindow('/ajax/add_image?blogid={$blog.id}&format=html&elemid=current-profile-image&replace=1');">Select New Image</button>

        <button onclick="return removeImage();">Remove Image</button>
