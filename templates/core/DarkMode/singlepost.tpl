...
<article class="ui grid post full-post">
    ...
    <header class="row">
        <div class="sixteen wide column">
            <h1 class="ui header post-title">
                {$post->title}
                <p class="sub header">Posted by <a href="{$blog->relativePath()}/author/{$post->author()->id}">{$post->author()->fullName()}</a> at {$post->timestamp|date_format}</p>
            </h1>
        </div>
    </header>
        ...
        {if count($post->tags) > 0}
            <div class="ten wide column post-tags">
                {foreach $post->tags as $tag}
                    ...
                    <a href="{$blog->relativePath()}/tags/{$tag}" class="ui black large tag label">{$caption}</a>
                {/foreach}
            </div>
        {/if}
    ...
    <div class="row post-navigation">
        ...
        {if gettype($previousPost) == "object"}
            <div class="eight wide column">
                <a href="{$blog->relativePath()}/posts/{$previousPost->link}" class="ui black labeled icon fluid button">
        ...
        {if gettype($nextPost) == "object"}
            <div class="eight wide column">
                <a href="{$blog->relativePath()}/posts/{$nextPost->link}" class="ui black right labeled icon fluid button">
    ...
    {if $userIsContributor}
        <div class="row contributor-actions">
            <div class="sixteen wide column">
                <a href="/cms/posts/edit/{$post->id}" class="ui black labeled icon button"><i class="edit icon"></i>Edit</a>
                <a href="/cms/posts/delete/{$post->id}" onclick='return confirm(\"Are you sure you want to delete this post?\");' class="ui black labeled icon button"><i class="trash icon"></i>Delete</a>
            </div>
        </div>
    {/if}
